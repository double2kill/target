...
describe('测试首页', () => {
    ...
    it(`表格包含了标题[${expectedTableTitle}]`, () => {
      ...
      //   const 计划完成时间 = dayjs()
      //     .add(index * 1.5, "hour")
      //     .subtract(subMin, "minute")
      //     .format("HH:mm");
      //   cy.get('[data-test-id="选择计划完成时间"]')
      //     .clear()
      //     .type(计划完成日期)
      //     .type("{enter}");
      //   cy.get('[data-test-id="计划完成时间"] input')
      //     .click()
      //     .type("{home}")
      //     .type(计划完成时间)
      //     .type(
      //       "{end}{backspace}{backspace}{backspace}{backspace}{backspace}{enter}"
      //     );
      //   cy.get('[data-test-id="添加按钮"]').click();
      // }

      //查看是否添加成功
      // cy.get(".n-data-table-table")
      //   .find("tr")
      //   .its("length")
      //   .should("eq", targets.length + 1);
      ...
      cy.get(".n-data-table-table")
        .find("tr")
        .each(item => {
          item.find('[data-test-id="完成"]').click()
        })
